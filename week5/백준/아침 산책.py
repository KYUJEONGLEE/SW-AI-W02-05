...
서현이는 산책을 위해 서울과학고의 지리를 분석했고, 그 결과 서울과학고를 N 개의 장소를 
N-1 개의 길이 잇는 트리 형태로 단순화시킬 수 있었습니다.
...
책 경로 위에 시작점과 끝점을 제외하고 실내 장소가 있으면 안 됩니다.
...
where = list(map(int, sys.stdin.readline().strip()))
